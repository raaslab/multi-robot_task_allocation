...
vector<max_min_lp_msgs::general_node> MaxMinLPCore::applyLocalAlgorithm() {
	////////////////////////////// Phase 1 //////////////////////////////
					...
					for (vector<string>::iterator itt = temp_blue_node_id.begin(); itt != temp_blue_node_id.end(); ++itt) {
						...
						for (int k = 0; k < temp_blue_pointer->second.loc_deg; k++) {
							if (temp_blue_pointer->second.loc_neighbor_id[k] == getNodeID(*it)) {
								...
								temp_target_pointer_verbal.push_back("(t"+boost::lexical_cast<string>(getNodeID(*it))
									+", "+boost::lexical_cast<string>(j)+", p"+boost::lexical_cast<string>(getNodeID(*itt))+")");
